...
req.kmdf-ver: 
req.umdf-ver: 
req.alt-api: RtlUnicodeStringCbCatN
...
ms.keywords: RtlUnicodeStringCbCatN
req.iface: 
---
...
## -description
<p>The <b>RtlUnicodeStringCbCatN</b> function concatenates two strings that are contained in <a href="https://msdn.microsoft.com/library/windows/hardware/ff564879">UNICODE_STRING</a> structures while limiting the size of the copied string.</p>


## -syntax

````
NTSTATUS RtlUnicodeStringCbCatN(
...
## -returns
<p><b>RtlUnicodeStringCbCatN</b> returns one of the following NTSTATUS values. </p><dl>
...
<p><b>RtlUnicodeStringCbCatN</b> returns the STATUS_INVALID_PARAMETER value when one of the following occurs:</p>
...
## -remarks
<p>The <b>RtlUnicodeStringCbCatN</b> function uses the destination buffer's size to ensure that the concatenation operation does not write past the end of the buffer. The function does <u>not</u> terminate the resultant string with a null character value (that is, with zero).</p>

<p>If the source and destination strings overlap, the behavior of the function is undefined.</p>

<p>The <i>SourceString</i> and <i>DestinationString</i> pointers cannot be <b>NULL</b>. If you need to handle <b>NULL</b> pointer values, use the <a href="https://msdn.microsoft.com/library/windows/hardware/ff562906">RtlUnicodeStringCbCatNEx</a> function.</p>
