...
Print processors must call <b>GdiResetDCEMF</b> whenever it is necessary to reset the printer's device context. The function must be called whenever the <a href="print.gdigetdevmodeforpage">GdiGetDevmodeForPage</a> function indicates that the current document page's <a href="display.devmodew">DEVMODEW</a> structure is not identical to that of the previous document page.
