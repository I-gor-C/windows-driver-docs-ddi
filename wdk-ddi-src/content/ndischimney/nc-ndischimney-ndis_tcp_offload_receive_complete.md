...
An offload target should use and return preposted receive requests in the order in which the requests
    were posted to the 
    <mshelp:link keywords="netvista.miniporttcpoffloadreceive" tabindex="0"><i>
    MiniportTcpOffloadReceive</i></mshelp:link> function of the offload target. That is, the offload target should use
...
An offload target must serialize calls to the 
    ...
    NdisTcpOffloadReceiveHandler</b></mshelp:link> function on a per -connection basis.
...
To improve system performance, an offload target can create a linked list that contains the 
    ...
    calls to its 
    <mshelp:link keywords="netvista.miniporttcpoffloadreceive" tabindex="0"><i>
    MiniportTcpOffloadReceive</i></mshelp:link> function. The offload target can pass such a linked list in a single call
    to the 
