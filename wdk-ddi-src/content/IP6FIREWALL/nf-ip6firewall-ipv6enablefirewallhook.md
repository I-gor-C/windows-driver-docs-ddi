---
UID: NF.ip6firewall.IPv6EnableFirewallHook
title: IPv6EnableFirewallHook
author: windows-driver-content
description: 
ms.assetid: 5ba481ae-fe94-4fe9-bc1b-3b1e961c4690
ms.author: windowsdriverdev
ms.date: 
ms.topic: function
ms.prod: windows-hardware
ms.technology: windows-devices
ms.keywords: IPv6EnableFirewallHook
req.iface: 
req.header: ip6firewall.h
req.include-header:
req.target-type:
req.target-min-winverclnt:
req.target-min-winversvr:
req.kmdf-ver:
req.umdf-ver:
req.lib:
req.dll:
req.irql: 
req.ddi-compliance:
req.alt-api:
req.alt-loc:
req.unicode-ansi:
req.idl:
req.max-support:
req.namespace:
req.assembly:
req.type-library:
---


# IPv6EnableFirewallHook function


## -description



## -parameters

### -param FirewallHookFunction


## -returns

This function returns NTSTATUS.
The method returns STATUS_SUCCESS if the operation succeeds. Otherwise, it may return an appropriate NTSTATUS Values error code. For more information, see [XREF-LINK:NTSTATUS Values].
	

## -remarks


## -irql




## -see-also